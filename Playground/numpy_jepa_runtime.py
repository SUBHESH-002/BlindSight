...
def predict_frames(model, frames: np.ndarray, query: str, top_k: int = 5):
    ...
    """
    # 1. Resize frames from 256x256 down to the 16x16 your NumPy model expects
    resized_frames = []
    # Take 5 evenly spaced frames from the segment
    indices = np.linspace(0, len(frames)-1, 5, dtype=int)
    for idx in indices:
        small_frame = cv2.resize(frames[idx], (16, 16), interpolation=cv2.INTER_AREA)  
        resized_frames.append(small_frame)
        
    X_v = np.array(resized_frames) / 255.0  # Normalize if your notebook expects 0-1
    X_v = X_v.reshape(1, 5, 16, 16, 3)      # Batch dimension
    ...
    # 5. Calculate Similarities against all possible action mappings
    similarities = []
    ...
    scores = (exp_sim / np.sum(exp_sim)).tolist()
