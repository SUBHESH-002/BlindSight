...
# ==========================================
# 7. SSV2 DATASET INTEGRATION & TRAINING
# ==========================================
import numpy as np
import pandas as pd
import ast
...
def load_and_train_ssv2(csv_path, num_classes=15, epochs=20, batch_size=4):
    print(f"Loading SSv2 metadata from {csv_path}...")
    df = pd.read_csv(csv_path)
    
    unique_classes = df['target_class'].unique()[:num_classes]
    df = df[df['target_class'].isin(unique_classes)]
    
    text_mapping = {}
    vocab_words = set(["describe", "action", "<PAD>"])
    
    for c in unique_classes:
        sample_label = df[df['target_class'] == c]['text_label'].iloc[0].lower()
        words = sample_label.split()
        text_mapping[c] = words
        vocab_words.update(words)
        
    vocab = {"<PAD>": 0}
    idx = 1
    for w in vocab_words - {"<PAD>"}:
        vocab[w] = idx
        idx += 1
    ...
    print(f"Classes: {text_mapping}")
    
    embed_dim = 256
    target_dim = 64
    
    text_encoder = BatchedSelfAttentionYEncoder(vocab_size=vocab_size, max_seq_len=20, embed_dim=embed_dim, target_dim=target_dim)
    # Match SSv2 shape (8, 64, 64, 3)
    vision_encoder = NumPyVJEPAEncoder(frame_size=64, patch_size=16, frames=8, embed_dim=embed_dim, depth=2, heads=4)
    ...
    loss_history = []
    print("Starting SSv2 Training Loop...")
    
    for epoch in range(epochs):
        df_train_shuffled = train_df.sample(frac=1).reset_index(drop=True)
        epoch_loss = 0
        num_batches = 0
        
        for start_idx in range(0, len(df_train_shuffled), batch_size):
            end_idx = min(start_idx + batch_size, len(df_train_shuffled))
            batch_df = df_train_shuffled.iloc[start_idx:end_idx]
            if len(batch_df) < 2: continue
            
            X_v_batch, text_query_batch, text_tokens_batch = [], [], []
            for _, row in batch_df.iterrows():
                npy_path = row['npy_path'].replace('\\', '/')
                try:
                    feat = np.load(npy_path)
                    if feat.shape != (8, 64, 64, 3):
                        continue
                    X_v_batch.append(feat)
                    text_query_batch.append([vocab[w] for w in ["describe", "action"]])
                    text_tokens_batch.append([vocab[w] for w in text_mapping[int(row['target_class'])]])
                except Exception as e:
                    continue
                    
            if len(X_v_batch) < 2: continue
            
            X_v_batch = np.array(X_v_batch)
            max_len = max(len(t) for t in text_tokens_batch)
            padded_tokens = [t + [0] * (max_len - len(t)) for t in text_tokens_batch]
            text_tokens_batch = np.array(padded_tokens)
            text_query_batch = np.array(text_query_batch)
    ...
    plt.title("VL-JEPA SSv2 Training Loss")
    plt.grid(True)
    plt.show()
    
    return model, text_mapping, vocab

if __name__ == '__main__':
    # Train predictor with SSv2 classes
    ssv2_csv = "ssv2_metadata.csv"
    ssv2_model, ssv2_mapping, ssv2_vocab = load_and_train_ssv2(ssv2_csv, num_classes=15, epochs=20, batch_size=4)
